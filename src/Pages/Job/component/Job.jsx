...
const Job = () => {
  ...
  return (
    <div className='w-full min-h-[100vh] flex  flex-col justify-center items-center pt-[60px]'>
      ...
      <div className='w-full min-h-[80vh] flex flex-wrap justify-start items-start gap-5'>
        {jobs ? jobs.map((e, i) => (
          <div key={i} className='w-[150px] lg:w-[22vw] h-[250px] lg:h-[200px] flex flex-col items-start justify-evenly p-4 shadow-lg'>

            <h2 className='font-bold text-2xl'>{e.title}</h2>
            <p className='text-gray-400'>{e.category}</p>
            <p className='hidden md:block'>{e.location}</p>
      ...
      {/* BUTTON POST NEW JOB */}
      <div onClick={() => navigate("/jobForm")} className='fixed bottom-5 right-10 bg-teal-500 px-4 py-2 rounded-lg shadow-lg cursor-pointer' >Post New Job</div>
